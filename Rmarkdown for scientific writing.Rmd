...
2.  根据Xie Yihui的经验，Windows中安装R注意几点^[https://bookdown.org/yihui/r-ninja/setup.html#configure-r]：    
    - 把默认安装目录中的R版本号去掉，比如R默认装在C:/Program Files/R/R-3.5.3/，不要接受这个结果，把它改成C:/Program Files/R/，以后升级软件仍然安装在这个文件夹；    
    - 不能安装在中文目录下；    
    - 用自定义方式安装。不要安装翻译文件。        
3. 配置`.Renviron`文件。这个文件是为R自身设置一些环境变量的，这里面的环境变量仅仅对R有效，不改变操作系统的设置。`.Renviron`文件放在用户目录下，也就是~/。如果文件不存在，可以用如下命令自动创建: `file.edit('~/.Renviron')`。这个文件可以直接打开编辑，推荐放一个环境变量在里面，就是`R_LIBS_USER="~/R"`，它指定R的软件包安装目录在`~/R`。重启R，以后安装R包就优先安装到`~/R`目录下。因为这个目录是固定的，所以即使将来升级R版本，R也会使用这个目录装包，你再也不必更新关于包的配置。
...
解决办法：    
  1. 清空 `C:\texlive\2016\texmf-var\fonts\cache` 中的文件；    
...
knitr::write_bib(c('knitr','bookdown','tidyverse','ggplot2'), 'mypackages.bib', width = 60)
```
...
登进Github. 新建一个仓库（或打开旧仓库），点开`Clone or Download`，右边拷贝SSH clone URL(注意不是https url!)。

```{r out.width='50%', fig.cap='拷贝SSH地址到RStudio'}
...
```{r out.width='75%', fig.cap='拷贝SSH地址到RStudio'}
