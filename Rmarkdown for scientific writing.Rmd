...
到如下网址下载最新版R并安装： https://www.r-project.org/ 。一般默认安装即可，不过，根据Xie Yihui的经验，Windows中安装R如下注意几点会更好^[https://bookdown.org/yihui/r-ninja/setup.html#configure-r]：    
  ...
  - 不能安装在中文目录下；    
  - 用自定义方式安装。不要安装翻译文件。          
  - 配置`.Renviron`文件。这个文件是为R自身设置一些环境变量的，这里面的环境变量仅仅对R有效，不改变操作系统的设置。`.Renviron`文件放在用户目录下，也就是~/。如果文件不存在，可以用如下命令自动创建: `file.edit('~/.Renviron')`。这个文件可以直接打开编辑，推荐放一个环境变量在里面，就是`R_LIBS_USER="~/R"`，它指定R的软件包安装目录在`~/R`。重启R，以后安装R包就优先安装到`~/R`目录下。因为这个目录是固定的，所以即使将来升级R版本，R也会使用这个目录装包，y以后再也不必更新关于包的配置。  
...
至此，已经可以完美运行R和纯英文的RMarkdown了。不过，如果还要用中文撰写RMarkdown文档，则需要安装Latex。
